		...
		if(jsonStr!=null&&jsonStr.length()>0){
			...
			if(user_name!=null&&!user_name.equals("")){
				...
				if(cust==null){
					...
					String validCode = SMSUtils.genrateCode(6);
					
					/*String url = "http://222.73.117.158/msg/";// 应用地址
					String account = "jiekou-clcs-03";// 账号
					String pswd = "Admin888";// 密码
					String mobile = user_name;// 手机号码，多个号码使用","分割
					String msg = "您好，您的验证码是"+validCode;// 短信内容
					...
					HttpSender.aliSend(user_name, "{\"code\":\""+validCode+"\",\"product\":\"出版头条\"}", "SMS_3480221", "注册验证");
		...
		if(jsonStr!=null&&jsonStr.length()>0){
			...
			if(user_name!=null&&!user_name.equals("")){
				...
				if(cust!=null){
					...
					//validCode = Md5.getMD5(validCode);
					HttpSender.aliSend(user_name, "{\"code\":\""+validCode+"\",\"product\":\"出版头条\"}", "SMS_3480223", "登录验证");
					map.put("data", validCode);
				}else{
					map.put("result", "003");
