...
<html>
	<head>
		...
		<title><c:if test="${type==1 }">创建</c:if><c:if test="${type==2 }">修改</c:if>反馈类型</title>
	...
	<body>
		<div id="slider_content" class="content">
			<div class="content_div">
				<h1>
					...
					<span class="action-span1 action-span2">-创建反馈类型</span>
				</h1>
				<form action="<%=path %>/admin/feed/type/addOrUpdateBackFeedType" method="post" onsubmit="return checkForm();">
					<input type="hidden" name="springMVC.token" value="${TOKEN}">
					<input type="hidden" name="type" value="${type}">
                    <input type="hidden" name="id" value="${backFeedType.id }"/>
				<div class="main-div">
					<table>
				    	
						<tr>
							<td class="label">反馈类型名称：</td>
							<td><input id="feed_type_name" name="feed_type_name"  type="text" value="${backFeedType.feed_type_name }" class="input" /><span class="require-field">*</span></td>
						</tr>
						<tr>
						    <td class="label">
							   	 &nbsp;
		...
		<script type="text/javascript">
			...
			function checkForm(){
				...
				if(isEmpty(feed_type_name)){
					layer.tips('反馈类型名称不能为空！', '#feed_type_name', {
					    tips: 2,
					    time:4000
					});
					return false;
