...
import cn.com.shukaiken.aop.ControllerLog;
...
public class InfoController {
	...
	//@ControllerLog(category=Constant.MODULE_NEWINFO,type=ControllerLog.SELECT,description="查询资讯列表 ")
	...
	//@ControllerLog(category=Constant.MODULE_NEWINFO,type=ControllerLog.SELECT,description="获取资讯审核列表")
	public String getInfoForFirstCheck(RequestContext rc,Info info,ModelMap model){
		...
		statusList.put("复核失败",Constant.INFO_STATUS_SECOND_CHECK_FAIL);
		model.addAttribute("statusMap", statusList);
		if(ValidateUtils.isEmpty(info.getStatus())){
			info.setStatusMap(statusList);
		}
	...
	//@ControllerLog(category=Constant.MODULE_NEWINFO,type=ControllerLog.SELECT,description="获取资讯复核列表")
	public String getInfoForSecondCheck(RequestContext rc,Info info,ModelMap model){
		logger.info("获取资讯复核列表:status="+info.getStatus());
		info.setIsValid(Constant.VALID);
		Map<String,String> statusList = new HashMap<String,String>(); 
		info.setIsValid(Constant.VALID);
		...
		model.addAttribute("page",page);
		model.addAttribute("queryInfo", info);
		model.addAttribute("type", "recheck");
		return INFO_PATH+"info-check-list";
	}
	
	//跳转到发布列表页，资讯配置
	@RequestMapping(value="/admin/info/getToPublishInfos")
	//@ControllerLog(category=Constant.MODULE_NEWINFO,type=ControllerLog.SELECT,description="获取资讯发布列表")
	...
	//@ControllerLog(category=Constant.MODULE_NEWINFO,type=ControllerLog.SELECT,description="获取资讯待排版列表")
	...
	//@ControllerLog(category=Constant.MODULE_NEWINFO,type=ControllerLog.SELECT,description="搜索资讯列表")
	...
	@ControllerLog(description="进入设置首页Banner页面",category=Constant.MODULE_NEWINFO,type=ControllerLog.OTHERS)
	@RequestMapping(value="/admin/info/setIndexBanner")
	public String setIndexBanner(RequestContext rc,ModelMap model){
		logger.info("进入设置首页Banner页面，操作人:"+rc.getCurrUser().getName()+",userId="+rc.getCurrUser().getId()+",进入时间："+DateTimeUtils.currentDateTime());
	...
	@ControllerLog(description="设置资讯首页图片",category=Constant.MODULE_NEWINFO,type=ControllerLog.ADD)
	...
	public Map<String,String> saveIndexBanner(RequestContext rc,HttpServletRequest request,ModelMap model){
		...
		logger.info("设置资讯首页图片，操作人:"+rc.getCurrUser().getName()+",userId="+rc.getCurrUser().getId()+",进入时间："+DateTimeUtils.currentDateTime());
		try {
			...
			qBan.setIsValid(Constant.VALID);
			List<InfoBanner> banners = infoBannerService.getIndexBannnerForBackByInfoId(qBan);
			if(banners!=null && banners.size()>0 && "2".equals(ischangeImage)){
				//先将其中的所有的都置为无效
				for(int i = 0;i<banners.size();i++){
	...
	@ControllerLog(description="进入新增资讯页面",category=Constant.MODULE_NEWINFO,type=ControllerLog.OTHERS)
	...
	@ControllerLog(description="创建新闻资讯",category=Constant.MODULE_NEWINFO,type=ControllerLog.ADD)
	...
	public Map<String,String> addInfo(RequestContext rc,Info info,HttpServletRequest request,HttpServletResponse response,ModelMap model){
		logger.info("后台创建资讯，创建人:"+rc.getCurrUser().getName()+",userId="+rc.getCurrUser().getId()+",创建时间:"+DateTimeUtils.currentDateTime());
		...
		try {
			if(info!=null){
				if(info.getInfoId()==null){
					...
					info.setAuthorId(rc.getCurrUser().getId());//作者ID
					info.setAuthorName(rc.getCurrUser().getName());
					info.setAuthorType("system");
					...
					int rows = infoService.modifyInfo(info, request, response, ischangeImage, bannerIds, tagIds);
					if(rows<=0){
						resultMap.put("fail", "true");
						if(Constant.INFO_STATUS_DRAFT.equals(info.getStatus())){
							resultMap.put("message", "草稿保存失败");
	...
	@ControllerLog(description="查看资讯",category=Constant.MODULE_NEWINFO,type=ControllerLog.OTHERS)
	...
	@ControllerLog(description="进入修改资讯页面",category=Constant.MODULE_NEWINFO,type=ControllerLog.OTHERS)
	...
	@ControllerLog(description="修改资讯",category=Constant.MODULE_NEWINFO,type=ControllerLog.UPDATE)
	...
	public Map<String,String> modifyInfo(RequestContext rc,Info info, HttpServletRequest request,HttpServletResponse response,ModelMap model){
		Map<String,String> map = new HashMap<String,String>();
		try {
			logger.info("修改资讯:修改人:"+rc.getCurrUser().getName()+",userId="+rc.getCurrUser().getId()+",修改时间:"+DateTimeUtils.currentDateTime());
			String ischangeImage =request.getParameter("ischangeImage");
			String bannerIds = request.getParameter("bannerIds");
			String tagIds = request.getParameter("tagIds");
			if(info!=null){
	...
	@ControllerLog(description="删除资讯",category=Constant.MODULE_NEWINFO,type=ControllerLog.DELETE)
	...
	public Map<String,String> deleteInfo(RequestContext rc){
		...
		try {
			String infoId = rc.get("infoId");
			if(!ValidateUtils.isEmpty(infoId)){
				logger.info("删除资讯:infoId="+infoId+",操作人:"+rc.getCurrUser().getName()+",userId="+rc.getCurrUser().getId()+",操作时间:"+DateTimeUtils.currentDateTime());
	...
	//分配资讯
	@ControllerLog(description="分配资讯",category=Constant.MODULE_NEWINFO,type=ControllerLog.UPDATE)
	...
	public Map<String,String> distributeInfo(RequestContext rc,ModelMap model){
		...
		try {
			...
			if(!ValidateUtils.isEmpty(infoIds) && !ValidateUtils.isEmpty(labelId)){
				logger.info("分配资讯:infoIds="+infoIds+"lableId="+labelId+",操作人:"+rc.getCurrUser().getName()+",userId="+rc.getCurrUser().getId()+",操作时间:"+DateTimeUtils.currentDateTime());
				Info info = new Info();
	...
	//推荐资讯
	@ControllerLog(description="推荐资讯",category=Constant.MODULE_NEWINFO,type=ControllerLog.UPDATE)
	...
	public Map<String,String> recommendInfo(RequestContext rc,ModelMap model){
		...
		try {
			...
			if(!ValidateUtils.isEmpty(infoId) && !ValidateUtils.isEmpty(labelId)){
				logger.info("推荐资讯:infoId="+infoId+"lableId="+labelId+",操作人:"+rc.getCurrUser().getName()+",userId="+rc.getCurrUser().getId()+",操作时间:"+DateTimeUtils.currentDateTime());
				Info info = new Info();
	...
	@ControllerLog(description="上传图片",category=Constant.MODULE_NEWINFO,type=ControllerLog.ADD)
				...
				/*if(filesMap!=null && filesMap.size()>0){
					for(Entry<String, MultipartFile> entry:filesMap.entrySet()){*/
						...
						if(imageFile!=null&&!imageFile.isEmpty()){
							...
							String uploadFileName =  appURL+ConfigurationUtils.readValue("info.images.access.path", "config.properties")+DateTimeUtils.customDateTime(new Date(),"yyyyMMdd" ) + "/"+fileName;
/*							String uploadFileName =  request.getScheme()+"://"+request.getServerName()+":"+
									request.getServerPort()+ConfigurationUtils.readValue("info.images.access.path", "config.properties")+DateTimeUtils.customDateTime(new Date(),"yyyyMMdd" ) + "/"+fileName;
*/							boolean isSuc = this.upload(imageFile,path , fileName, request, model);
	...
	@ControllerLog(description="修改资讯状态",category=Constant.MODULE_NEWINFO,type=ControllerLog.CHECKED)
	...
	public Map<String,String> changeStatus(RequestContext rc,ModelMap model){
		...
		String message = "";
		logger.info("修改资讯的状态,infoId="+infoId+",to status="+status+",操作人:"+rc.getCurrUser().getName()+",userId="+rc.getCurrUser().getId()+",id=");
		try {
			if(!ValidateUtils.isEmpty(infoId)){
				Info info = new Info();
				info.setInfoId(Integer.valueOf(infoId));
				info.setStatus(status);
				int result = infoService.changeInfoStatusById(info,opinion,rc.getCurrUser());
